...
func createSegments(inputs []string) []Segment {
	segments := make([]Segment, len(inputs))

	lastPoint := Point{0, 0}

	for index, input := range inputs {
		dir, dis := parseInstruction(input)

		nextPoint := Point{lastPoint.X, lastPoint.Y}
		...
		segments[index] = Segment{lastPoint, nextPoint}
...
// func findBounds(inputs []string) (top int, bottom int, left int, right int) {
...
func parseInstruction(instruction string) (direction string, distance int) {
	...
	direction = string(runes[:1])
	distance, _ = strconv.Atoi(string(runes[1:len(runes)]))
